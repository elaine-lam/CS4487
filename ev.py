    ...
    for root, dirs, files in os.walk(path):
        for name in files:
            if fnmatch.fnmatch(name, "*.jpg"):
                labels.append(0 if 'nature' in root else 1)
...
if __name__ == "__main__":
    ...
    test_path = r'AIGC-Detection-Dataset\val'
    ...
    for folder in folders:
        ...
        for model_path in model_paths:
            ...
            print(f'Model: {model_path}, Test accuracy: {test_accuracy}')
        
    with open('results2.csv', 'w', newline='') as csvfile:
        fieldnames = ['Model',  'Test Accuracy']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        for i in range(len(path)):
            writer.writerow({'Model': path[i], 'Test Accuracy': test_acc[i]})
